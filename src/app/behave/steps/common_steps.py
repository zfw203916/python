...
STUDENT_MANAGE_DIR = APP_DIR / 'clients' / 'studentmange'  # src/app/clients/studentmanage
...
def step_common_error_message(context, message):
    ...
    if response.status_code == 200:
        ...
        # 对于非200状态码，直接检查状态码
        assert response.status_code in [400, 401, 404], f"期望错误状态码，实际: {response.status_code}"
        try:
            data = response.json()
            error_msg = data.get("message") or data.get("error") or ""
            assert message in error_msg, f"期望消息 '{message}' 不在 '{error_msg}' 中"
