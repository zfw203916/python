...
def load_users_from_csv(file_path: str) -> List[Dict[str, str]]:
    ...
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            ...
            has_header = (
                "username" in first_line.lower() and "password" in first_line.lower()
            )
...
# ==================== 批量注册 ====================
def batch_register(
    users: List[Dict[str, str]], base_url: str, verify_login: bool = False
) -> Dict[str, int]:
    """
    批量注册用户
    """
...
def generate_test_users_csv(
    file_path: str, count: int = 100, with_password: bool = True
):
    """
    生成指定数量的测试用户CSV文件
    """
...
def main():
    ...
    if not users:
        ...
        print("   2. 每行格式: username,password")
        print("   3. 或运行: python batch_register.py --generate 100")
        return
