...
class PerformanceMonitor:
    ...
    def collect_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": ((total.num_requests - total.num_failures) / 
                           total.num_requests * 100) if total.num_requests > 0 else 0,
        ...
        for attempt in range(Config.MAX_RETRIES):
            try:
                with self.client.post(
                    ...
                    # 断言：会话已建立
                    assert "logged_in" in self.client.cookies or \
                           "session" in self.client.cookies, \
                        "登录会话未建立"
        ...
        try:
            ...
            with self.client.delete(
                ...
                if verify_response.status_code == 404:
                    ...
                    delete_response.failure(f"删除未生效: {verify_response.status_code}")
                ...
                with self.client.post(
                    ...
                    if response.status_code == 400 or "用户不存在" in response.text:
                        ...
                        if register_resp.status_code in [200, 201]:
                            ...
                            logger.error(f"❌ 用户 {username} 注册失败: {register_resp.status_code}")
...
def on_test_stop(environment, **kwargs):
    ...
    if environment.runner and environment.runner.stats:
        ...
        if total.num_requests > 0:
            ...
            logger.info(f"   中位数响应时间: {total.get_response_time_percentile(0.50):.2f}ms")
