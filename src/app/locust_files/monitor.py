...
class PerformanceMonitor:
    ...
    def start_locust_webui(self, test_file: str = "locustfile.py"):
        ...
        try:
            ...
            if os.name == "nt":  # Windows
                ...
                subprocess.Popen(
                    ...
                    if os.system("which gnome-terminal > /dev/null 2>&1") == 0
                        ...
                        "osascript",
                        "-e",
                        f'tell app "Terminal" to do script "locust -f {test_file} --host http://localhost:5003"',
                    ]
                    if os.name == "posix"
            ...
            print(
                f"⚠️ 自动启动失败，请手动运行: locust -f {test_file} --host=http://localhost:5003"
            )
            print(f"错误: {e}")
        ...
        try:
            while True:
                ...
                if stats:
                    ...
                    success_rate = (
                        ((num_requests - num_failures) / num_requests * 100)
                        if num_requests > 0
                        else 100
                    )
                    ...
                    print(
                        f"{timestamp:<20} {num_requests:<10} {num_failures:<10} "
                        f"{success_rate:>6.2f}%   "
...
def main():
    ...
    parser.add_argument(
        "--host", default="http://localhost:8089", help="Locust Web UI地址"
    )
    ...
    try:
        monitor.monitor(
            interval=args.interval, duration=args.duration, auto_start=args.auto_start
        )
