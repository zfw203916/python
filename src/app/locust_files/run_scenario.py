...
def run_scenario(
    ...
    html_report = os.path.join(
        Config.REPORT_DIR, f"report_{scenario_name}_{timestamp}.html"
    )
...
def list_scenarios(config_file: str = Config.SCENARIOS_FILE):
    ...
    for name, scenario in config["scenarios"].items():
        print(
            f"{name:<15} {scenario['description']:<35} "
    ...
    while True:
        ...
        if choice == "1":
            ...
            for name in config["scenarios"].keys():
                ...
                run_scenario(
                    name, test_script=Config.TEST_SCRIPT_BASIC, wait_service=False
                )
                logger.info(f"{'=' * 60}")
        elif choice == "5":
            logger.info("\n🌐 启动 Web UI 模式...")
            ...
            if report_dir.exists():
                reports = sorted(
                    report_dir.glob("*.html"), key=os.path.getmtime, reverse=True
                )
                if reports:
                    logger.info("\n📊 最近的报告:")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        if command == "list":
            ...
            test_script = (
                Config.TEST_SCRIPT_ADVANCED
                if use_advanced
                else Config.TEST_SCRIPT_BASIC
            )
