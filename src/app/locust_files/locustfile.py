...
from utils import get_logger, generate_test_data
...
class TestData:
    ...
    # 中文姓名库 - 用于生成测试数据
    SURNAMES = ["张", "李", "王", "刘", "陈", "杨", "黄", "周", "赵", "吴", "徐", "孙", "马", "朱", "胡"]
    GIVEN_NAMES = ["伟", "芳", "秀英", "敏", "静", "丽", "强", "磊", "洋", "勇", "艳", "杰", "娜", "军", "涛"]
    ...
    def on_start(self):
        ...
        with self.client.post(
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"❌ 用户 {self.username} 登录请求失败: {response.status_code}")
        ...
        try:
            ...
            with self.client.get(
                ...
                if search_response.status_code == 200:
                    ...
                    search_response.failure(f"学号搜索失败: {search_response.status_code}")
        ...
        try:
            ...
            with self.client.get(
                ...
                if detail_response.status_code == 200:
                    ...
                    detail_response.failure(f"查看详情失败: {detail_response.status_code}")
        except Exception as e:
        ...
        try:
            ...
            with self.client.put(
                ...
                if update_response.status_code == 200:
                    ...
                    update_response.failure(f"更新请求失败: {update_response.status_code}")
        except Exception as e:
        ...
        try:
            ...
            with self.client.delete(
                ...
                if delete_response.status_code == 200:
                    ...
                    delete_response.failure(f"删除请求失败: {delete_response.status_code}")
        except Exception as e:
...
class StudentManagementUser(HttpUser):
    ...
    def on_start(self):
        """用户启动时的钩子"""
        logger.info(f"👤 虚拟用户启动")
    
    def on_stop(self):
        """用户停止时的钩子"""
        logger.info(f"👤 虚拟用户停止")
...
def on_test_stop(environment, **kwargs):
    ...
    if environment.runner and environment.runner.stats:
        ...
        logger.info(f"   95% 响应时间: {total.get_response_time_percentile(0.95):.2f}ms")
        logger.info(f"   99% 响应时间: {total.get_response_time_percentile(0.99):.2f}ms")
        ...
        if stats.entries:
            ...
            for name, stat in stats.entries.items():
                if stat.num_requests > 0:
                    ...
                    logger.info(f"      95%响应: {stat.get_response_time_percentile(0.95):.2f}ms")
