...
def api_client(settings:Settings) -> ApiClient:
     ...
     client = ApiClient(settings=settings) # ✅ 传实例 settings，不是类 Settings,之前写错了。 ApiClient(settings=Settings)
     yield client
     client.close()
